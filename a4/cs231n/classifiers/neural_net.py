...
class TwoLayerNet(object):
  ...
  def _relu(self,X):
    """
    Implementation of RELU function for 1D numpy array
   ...
   # y = list(map(lambda x: max(x, 0), X))    
    return np.maximum(X,0)

  def _softmax(self,X):
    """
    Implementation of softmax function for 1D numpy array
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    # python的numpy array运算有个broadcast功能，它会自动扩维。不放心的话，你可以查一下broadcast机制
    first_layer_output=np.dot(X,W1)+b1
    activated_result=self._relu(first_layer_output)
    ...
    #############################################################################
    #for back propogation purpose we need save more
    ...
    backward_first_layer_output=backward_activated_result*(first_layer_output>0)
